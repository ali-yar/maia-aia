...
namespace aia
{
	...
	********************************************
	...
	inline void imshow(const std::string winname, const cv::InputArray arr, bool wait = true, float scale = 1.0)
	{
		// create window
	...
	throw (aia::error)
	{
		...
		double fps = inputPath.empty() ? CAMERA_FPS : capture.get(CV_CAP_PROP_FPS);
		...
		if(!outputPath.empty())
		{
			output.open(outputPath, CV_FOURCC('M','J','P','G'), fps, cv::Size((int)(capture.get(CV_CAP_PROP_FRAME_WIDTH)),(int)(capture.get(CV_CAP_PROP_FRAME_HEIGHT))), true);
			if (!output.isOpened())
		...
		bool stop(false);
		int delay = inputPath.empty()? 1 : static_cast<int>(1000/fps); // delay between each frame in ms
		while (!stop) 
		{
	...
	inline std::vector < cv::Rect > faceDetector(const cv::Mat& frame) throw (aia::error)
	{
		...
		face_cascade.detectMultiScale(frameCopy, faces, 1.1, 3, CV_HAAR_FIND_BIGGEST_OBJECT|CV_HAAR_SCALE_IMAGE, cv::Size(30,30));
